...
public class Ex01_String {
 ...
 // 문자열 객체는 항상 새로 생성된다.
  ...
  public static void ex03() {
    ...
    boolean isEqual3 = name1.equalsIgnoreCase(name3);  // 대소문자 구분하지 않고 문자열 비교. -> tom == Tom -> true반환.
  ...
  public static void ex08() {
    ...
    int idx4 = statement.indexOf("걱정", idx3 + 1); // -1
    System.out.println("첫번째: " + idx1 + "\n두번째: " + idx2 + "\n세번째: " + idx3 + "\n네번째: " + idx4);
  ...
  public static void ex10() {
    ...
    // @가 포함되어 있는가?
    if(email.contains("@")) {
      System.out.println("@ 포함 되어 있다.");
    } else {
      System.out.println("@ 미포함 되어 있다.");
    }
  ...
  public static void ex11() {
    ...
    // isBlack 메소드
    // 1. 빈 문자열이거나, 공백 문자로 구성되었다면 true 반환 아니면 false반환
    // 2. JDK 11이후 버전에서 사용 가능
    if(name.isBlank()) {
      System.out.println("이름이 없다.");
    } else {
      System.out.println("이름이 있다.");
  ...
  public static void ex12() {
    ...
    System.out.println(String.format("%o", number));  // %o 8진수로 표지 
    ...
    // 출력 폭 조정
    System.out.println(String.format("%5d", number));  // 5자리 10진수로 표기, 숫자를 오른쪽 정렬 처리
    System.out.println(String.format("%-5d", number)); // 5자리 10진수로 표기, 숫자를 왼쪽 정렬 처리
    System.out.println(String.format("%05d", number));  // 5자리 10진수로 표기, 숫자를 오른쪽 정렬 처리, 빈 자리 0채움
    
    System.out.println(String.format("%10s", str));  // 10자리 문자열 표시, 문자열 오른쪽 정렬 처리
    System.out.println(String.format("%-10s", str)); // 10자리 문자열 표시, 문자열 왼쪽 정렬 처리
  ...
  public static void ex15() {
    ...
    String str = ip.replaceAll(".", "_"); // 192_168_0_101을 기대하지만 정규식 패턴에서 .은 모든 문자를 의미하므로 원하는 결과를 얻을 수 없다.
    System.out.println(str);
