...
public class MainWrapper {
     ...
     * 2. 배열에 모든 변수가 이름은 같지만, 인덱스가 다르다.
     * 3. 인덱스란 배열의 모든 변수를 구붆기 위한 위치 정보이다. 0부터 1씩 증가
     * 4. 기본 용어
     *    1) 배열명 : 모든 변수의 대표 이름
  ...
  public static void ex02() {
    ...
    for(int i = 0; i < length; i++) {
      System.out.println(a[i]);  // a[0], a[1], ~ ,a[4] 순서대로 호출
    }
  ...
  /*
   * 배열의 초기화
   * 1. 배열을 생성하면 자동으로 초기화된다. (, 0.0, false, null)
...
//    }
  ...
  public static void ex08() {
    ...
    // 하지만, 배열의 길이를 변경(대부분 늘리는것) 하려면?? -> 초기화하고 늘리기? 내생각
    // 새로운 배열을 만들어서 기존배열의 데이터를 모두 옮긴디.
    ...
    */
    System.arraycopy(a, 0, b, 0, a.length); // 배열 a의 인덱스 0부터 복사 시작 배열 b의 인덱스 0으로 보내기 시작
    
    // 기존 배열 a의 참고값을 새로운 배열 b의 참고값으로변경
    a = b;
    
    // 이제 더 이상 배열 a가 참조하던 0x10000000000 영역을 참조하는 변수가 없다.
    // 0x10000000000 영역의 메모니 누수가 발생한다.
    // 이런영역을 C++ 같은 언어에서는 개발자가 직접 메모리를 해제해야 한다.
    // JAVA는 스스로 메모리를 해제한다. (garbage collector가 담당 : System.gc())
    
    // 이제 더이상 배열a가 참조하던 0x10000000000 영역을 참조하는 변수가없다.   
    
    // 배열 a 확인 (단순 조회) -> Java 기능 이용
